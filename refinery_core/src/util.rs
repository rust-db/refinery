...
#[cfg(test)]
mod tests {
    use super::find_migration_files;
    use std::fs;
    ...
    fn finds_mod_migrations() {
        ...
        let mut mods: Vec<PathBuf> = find_migration_files(migrations_dir).unwrap().collect();
        mods.sort();
    ...
    fn ignores_mod_files_without_migration_regex_match() {
        ...
        let mut mods = find_migration_files(migrations_dir).unwrap();
