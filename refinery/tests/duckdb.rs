...
mod ddb {
    ...
    use refinery_core::duckdb::Error;
    use refinery_core::duckdb::{Connection, OptionalExt};
    ...
    fn migrates_from_config() {
        ...
        let path = db.path().join("test.db");
        let mut config = Config::new(ConfigDbType::Duckdb).set_db_path(path.to_str().unwrap());
