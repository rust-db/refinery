        ...
        ConfigDbType::Mssql | ConfigDbType::Turso => {
            cfg_if::cfg_if! {
                ...
                if #[cfg(any(feature = "mssql", feature = "turso"))] {
                    ...
                    panic!("tried to migrate async from config for a {:?} database, but its matching feature was not enabled!", config.db_type());
                }
            }
        }
