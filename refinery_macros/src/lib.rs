...
fn migration_fn_quoted<T: ToTokens>(_migrations: Vec<T>) -> TokenStream2 {
    let result = quote! {
        ...
        pub fn runner() -> Runner {
            let quoted_migrations: Vec<(&str, &str, String)> = vec![#(#_migrations),*];
            let mut migrations: Vec<Migration> = Vec::new();
            for module in quoted_migrations.into_iter() {
                migrations.push(Migration::unapplied(format!("{}.{}",module.0,module.1).as_str(), &module.2).unwrap());
            }
...
pub fn embed_migrations(input: TokenStream) -> TokenStream {
    ...
    for migration in migration_files {
        ...
        let file_stem = migration
            .file_stem()
            .and_then(|stem| stem.to_os_string().into_string().ok())
            .unwrap();
        let path = migration.display().to_string();
        let file_extension = migration
            .extension()
            .and_then(|ext| ext.to_os_string().into_string().ok())
            .unwrap();

        if file_extension == "sql" {
            _migrations
                .push(quote! {(#file_stem, #file_extension, include_str!(#path).to_string())});
        } else if file_extension == "rs" {
            let rs_content = fs::read_to_string(&path)
                .unwrap()
                .parse::<TokenStream2>()
                .unwrap();
            let ident = Ident::new(&file_stem, Span2::call_site());
            ...
            _migrations.push(quote! {(#file_stem, #file_extension, #ident::migration())});
...
mod tests {
    ...
    #[test]
    fn test_quote_fn() {
        let migs = vec![quote!("V1__first", "sql", "valid_sql_file")];
        let expected = concat! {
            "use refinery :: { Migration , Runner } ; ",
            "pub fn runner () -> Runner { ",
            "let quoted_migrations : Vec < (& str , & str , String) > = vec ! [\"V1__first\" , \"sql\" , \"valid_sql_file\"] ; ",
            "let mut migrations : Vec < Migration > = Vec :: new () ; ",
            "for module in quoted_migrations . into_iter () { ",
            "migrations . push (Migration :: unapplied (format ! (\"{}.{}\" , module . 0 , module . 1) . as_str () , & module . 2) . unwrap ()) ; ",
            "} ",
